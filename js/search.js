  ...
  open() {
    ...
    const update = () => {
      ...
      Store.all('decisions').filter(d => match(d.title) || match(d.context) || match(d.decision))
        .slice(0, 6).forEach(d => items.push({ kind: 'Decision', label: d.title, meta: DECISION_STATUS_LABELS[d.status] || '', onpick: () => { closeModal(); App.go('decisions'); Decisions.openEditor(d); } }));
